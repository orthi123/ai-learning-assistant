...
const DashboardPage = () => {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        // Fix: Service response handle kora hocche
        // Response structure jodi { success, data: { overview... } } hoy
        if (response?.data) {
  ...
  // Overview mapping safely dhora hocche
  ...
  return (
    <div className="min-h-screen p-6">
      <div className="fixed inset-0 bg-[radial-gradient(#e5e7eb_1px,transparent_1px)] bg[length:16px_16px] opacity-30 pointer-events-none" />
              ...
              {[
                ...(dashboardData.recentActivity.documents || []).map(
                  (doc) => ({
                    ...
                    timestamp:
                      doc.lastAccessed || doc.uploadDate || doc.createdAt,
                ...
                .map((activity, index) => (
                  <div
                    ...
                    className="flex items-center justify-between p-4 rounded-xl bg-slate-50/50 border border-slate-200/60 hover:bg-white hover:border-slate-300/60 hover:shadow-md transition-all duration-200"
                  >
