      ...
      <nav className="flex gap-2">
        {tabs.map((tab) => (
          <button
            ...
            className={`relative pb-4 px-2 md:px-6 text-sm font-semibold transition-all duration-
            ${
              activeTab === tab.name
                ? "text-emerald-600"
          ...
          >
            ...
            {activeTab === tab.name && <div className="absolute inset-0 bg-gradient-to-b from bg-emerald-50/50 to-transparent rounded-t-xl -z-10" />}
          </button>
        ))}
      </nav>
    </div>
